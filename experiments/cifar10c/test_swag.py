...
def get_args():
    ...
    parser.add_argument('-tdd', '--train_data_dir',
                        help='The directory to find training dataset',
...
if __name__ == '__main__':
    ...
    for runfolder in sorted([d for d in listdir(args.traindir)
        ...
        args.train_data_dir, args.tvsplit, args.workers,
        (device != torch.device('cpu')), args.batch, args.batch)
        cifarc_loader = get_cifarc_loader(
            args.data_dir, args.workers, (device != torch.device('cpu')),
            args.batch, 1, CORRUPTION_CHOICES[args.corruptions],
            args.severity)
        ...
        log_ece.send((runfolder, prefix, len(cifarc_loader), outputsaver))
        with torch.no_grad():
            ...
            do_epoch(cifarc_loader, do_swagevalbatch, log_ece, device,
        ...
        with torch.no_grad():
            ...
            if args.swag_bnupdate:
                ...
                print(' Done.')
            swamodel.eval()
            do_epoch(cifarc_loader, do_evalbatch, log_ece, device,
                     model=swamodel)
        bins, _, avgvloss = log_ece.throw(StopIteration)[:3]
