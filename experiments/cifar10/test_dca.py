...
def get_dcaloader(outclass: int, args):
    ...
    if args.valdata:
        _, data_loader = TRAINDATALOADERS[dataset](
            ...
            args.testsamples, args.testsamples)
    else:
...
def get_dcwaloaders(outclass: int, args):
    dataset = {v: k for k, v in OUTCLASS.items()}[outclass]
...
if __name__ == '__main__':
    ...
    for runfolder in sorted([d for d in listdir(args.traindir)
        ...
        ckpt_path = pjoin(args.traindir, runfolder, 'checkpoint.pt')
        if not exists(model_path) or exists(ckpt_path):
        ...
        dca_loader = get_dcaloader(outclass, args)
        ...
        log_ece.send((runfolder, prefix, len(dca_loader), outputsaver))
        with torch.no_grad():
            dcamodel.eval()
            do_epoch(dca_loader, do_evalbatch, log_ece, device,
        ...
        prefix = 'dcwaval' if args.valdata else 'dcwatest'
        train_loader, dcwa_loader = get_dcwaloaders(outclass, args)
        ...
        log_ece.send((runfolder, prefix, len(dcwa_loader), outputsaver))
        with torch.no_grad():
            dcwamodel = dcamodel.wamodule()
            if args.dca_bnupdate:
                print('updating BatchNorm ...', end='')
                bn_update(train_loader, dcwamodel, device=device)
                print(' Done.')
            dcwamodel.eval()
            do_epoch(dcwa_loader, do_evalbatch, log_ece, device,
                     model=dcwamodel)
        bins, _, avgvloss = log_ece.throw(StopIteration)[:3]
