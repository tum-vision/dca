...
def asnpbatchiter(entryiter, batchsize: int, droplast: bool = False):
    if droplast:
        ...
        yield from (np.concatenate(b).reshape(-1, *b[0].shape) for b in
                    zip_longest(*[entryiter]*batchsize,
                                fillvalue=np.array(())))


@autoinitcoroutine
def coro_npybatchgatherer(
        filepath, entrycount: int, entryshape=(), overwrites=False,
        dtype='float'):
...
def npyiterator(filepath, transform=None, cache=1000000):
    ...
    pos = 0
    while pos * cache <= ll:
        chunk = np.load(filepath, mmap_mode='r')[pos*cache:(pos+1)*cache]
...
def npybatchiterator(filepath, batchsize: int, droplast: bool = False,
                     transform=None, cache=1000000):
