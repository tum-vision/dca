# Adapted from https://github.com/izmailovpavel/understandingbdl
...
class SWAG(torch.nn.Module):
    def __init__(self, basemodel: nn.Module, max_rank: int = 20):
        ...
        self.register_buffer('deviations',
                             torch.empty((0, self.num_parameters)))
    ...
    def to(self, *args, **kwargs):
        ...
        (device, dtype, non_blocking
         ) = torch._C._nn._parse_to(*args, **kwargs)[:3]
    ...
    def _sampled_model_modelwise(self, model=None) -> nn.Module:
        mean, var, rank, dev = (
            self.mean, self.variance, self.rank, self.deviations)
        if rank > 1:
