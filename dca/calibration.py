...
def data2bins(rightconfs: Iterable[Tuple[bool, float]], nbin: int = 10
              ) -> BINSTYPE:
    bincounts = [0] * nbin
    corrects = [0] * nbin
    cumconf = [0.0] * nbin
...
def joinbins(*binses):
    ...
    return ([sum(cs) for cs in zip(*bincountses)],
            [sum(cs) for cs in zip(*correctses)],
            [sum(cs) for cs in zip(*cumconfs)])


# plot and save reliability diagram
def bins2diagram(bins: BINSTYPE, displays: bool = False,
                 saveas: Optional[str] = None) -> None:
    nbin = len(bins[0])
    binvals = [float(i) / nbin for i in range(nbin + 1)]
    accconfs = [(float(corr)/bc, cconf/bc) if bc > 0 else (0.0, 0.0)
                for bc, corr, cconf in zip(*bins)]
    weights = ([acc for acc, _ in accconfs],
    ...
    _, _, ps = a1.hist([binvals[:-1], binvals[:-1]], binvals, weights=weights,
                       color=[(0, 0, 1, 1), (1, 0, 0, 0.5)],
                       label=('Empirical', 'Gap'), stacked=True)
    total = sum(bins[0])
    freqs = [float(c)/total for c in bins[0]]
    a2.set_ylim(0, 1)
    a2.set_ylabel('Frequency')
    a2.hist([binvals[:-1]], binvals, weights=[freqs], rwidth=0.3,
            color=[(0.5, 0.5, 0.5, 1)], label=('Frequency',))
